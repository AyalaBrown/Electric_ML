...
async function training(){
    ...
    for (const bus of uniqueIdtags) {
        ...
        const { mean: y_mean, variance: y_variance } = tf.moments(y, 0);
        const y_normalized = tf.div(tf.sub(y, y_mean), tf.sqrt(y_variance.add(1e-8)));
        ...
        const y_train = tf.gather(y_normalized, trainIndicesArray);
