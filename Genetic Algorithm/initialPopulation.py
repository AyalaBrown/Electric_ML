    ...
    for i in range(npop):
        ...
        for j in range(0, len(busses)):
            if(busses[j]["entryTime"] >0 and busses[j]["exitTime"]>0):
                ...
                price = calculate_price(ampere, startTime, endTime, prices)
...
def calculate_price(ampere, startTime, endTime, prices):
    ...
    for k in range(0, len(prices)):
        if (startTime >= sorted_prices[k]["from"] and startTime<prices[k]["to"]) or price > 0:
            if endTime <= sorted_prices[k]["to"]:
                price += sorted_prices[k]["finalPriceInAgorot"]/100 * ampere * (endTime/1000./60./60. - startTime/1000./60./60.) *650/1000
                return price
            else:
                if price == 0:
                    price = sorted_prices[k]["finalPriceInAgorot"]/100 * ampere * (sorted_prices[k]["to"]/1000./60./60. - startTime/1000./60./60.) *650/1000
                else:
                    price += sorted_prices[k]["finalPriceInAgorot"]/100 * ampere * (sorted_prices[k]["to"]/1000./60./60. - sorted_prices[k]["from"]/1000./60./60.) *650/1000
    return price